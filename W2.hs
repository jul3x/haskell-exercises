...
remove :: Int -> [a] -> [a]
remove i xs = take (i) xs ++ takeFinal' (length xs - i - 1) xs
...
countSorted ss = length (filter (\x -> sort x == x) ss )
...
funny :: [String] -> String
funny strings = map toUpper (intercalate " " (filter longerThan5 strings))
    where longerThan5 x = length x > 5
...
quicksort xs = 
    let to_sort = tail xs
        first' = head xs
    in quicksort (filter (\x -> x < first') to_sort) ++ [head xs] ++ quicksort (filter (\x -> x >= first') to_sort)
...
powers :: Int -> Int -> [Int]
powers n max = powers' n 1 max

powers' :: Int -> Int -> Int -> [Int]
powers' x y max
    | y > max = []
    | otherwise = [y] ++ powers' x (y*x) max
...
sums i
    | i == 0 = []
    | i == 1 = [1]
    | otherwise = new_sum ++ [last new_sum + i]
    where new_sum = sums (i-1) 
...
sorted [] = True
sorted [a] = True
sorted (x:xs) = if (x <= head xs) then sorted xs else False   
...
sumsOf' [] _ = []
sumsOf' (x:xs) add = [add + x] ++ sumsOf' xs (add + x)
...
mymaximum cmp def (x:xs) = if cmp x computed == LT then computed else x
    where computed = mymaximum cmp x xs
...
interpreter ss = interpreter' 0 0 ss

interpreter' a b [] = []
interpreter' a b ("printA":cmds) = show a:interpreter' a b cmds
interpreter' a b ("printB":cmds) = show b:interpreter' a b cmds
interpreter' a b ("incA":cmds) = interpreter' (a+1) b cmds
interpreter' a b ("incB":cmds) = interpreter' a (b+1) cmds
interpreter' a b ("decA":cmds) = interpreter' (a-1) b cmds
interpreter' a b ("decB":cmds) = interpreter' a (b-1) cmds
interpreter' a b (_:cmds) = "Error!":interpreter' a (b-1) cmds
...
squares :: Int -> [Integer]
squares n = take n [s | x <- [1..], let s = x^2, last (show (s)) == head (show (s))]